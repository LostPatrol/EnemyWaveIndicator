...
namespace {
uint64_t now = 0;
ewi::Callback queuedCallback = nullptr;
void* queuedContext = nullptr;
bool synchronous = false, reject = false;
ewi::ThreadSample sample{42, true, true};
unsigned calls = 0;
unsigned actions = 0;
uint32_t actionStatus = 3;
void action(ewi::ThreadSample& value) {
...
bool enqueue(ewi::Callback callback, void* context) {
    ++calls;
...
int main() {
    {
        ...
        ewi::DispatchProbe p; reset(p); actions = 0;
    ...
    }
    {
        ...
        ewi::DispatchProbe p; reset(p); actions = 0; actionStatus = 2;
        p.configure({enqueue, readThread, clockNow, action}); p.pump(); drain(); p.pump();
    ...
    }
    {
        ...
        ewi::DispatchProbe p; reset(p); synchronous = true; actionStatus = 2;
    ...
    }
    {
        ...
        unsigned char bytes[ewi::EngineThreadIdentity::SnapshotSize]{};
        bytes[0] = 42; bytes[16] = 1;
        auto result = ewi::EngineThreadIdentity::decode(bytes, sizeof(bytes), 42, false);
        REQUIRE(result.initialized && result.gameThread && result.expectedTid == 42 && !result.runtimeInitialized);
        REQUIRE(!ewi::EngineThreadIdentity::decode(bytes, sizeof(bytes), 43, true).gameThread);
        bytes[16] = 0; REQUIRE(!ewi::EngineThreadIdentity::decode(bytes, sizeof(bytes), 42, true).initialized);
        bytes[16] = 2; REQUIRE(!ewi::EngineThreadIdentity::decode(bytes, sizeof(bytes), 42, true).initialized);
        bytes[0] = 0; bytes[16] = 1; REQUIRE(!ewi::EngineThreadIdentity::decode(bytes, sizeof(bytes), 0, false).initialized);
        REQUIRE(!ewi::EngineThreadIdentity::decode(bytes, sizeof(bytes) - 1, 42, false).identityReadOk);
        REQUIRE(!ewi::EngineThreadIdentity::decode(nullptr, 0, 42, false).identityReadOk);
        unsigned char signature[sizeof(ewi::EngineThreadIdentity::Signature)];
        memcpy(signature, ewi::EngineThreadIdentity::Signature, sizeof(signature));
        REQUIRE(ewi::EngineThreadIdentity::matches(signature, sizeof(signature)));
        for (size_t i = 0; i < sizeof(signature); ++i) {
            signature[i] ^= 1; REQUIRE(!ewi::EngineThreadIdentity::matches(signature, sizeof(signature))); signature[i] ^= 1;
        }
        ewi::EngineThreadIdentity identity; identity.configure(nullptr);
        REQUIRE(!identity.available() && !identity.sample(42, false).identityReadOk);
    }
    {
        ewi::DispatchProbe p; reset(p); p.pump();
    ...
    {
        ewi::DispatchProbe p; reset(p); synchronous = true;
    ...
    {
        ewi::DispatchProbe p; reset(p); sample.identityReadOk = false; p.pump(); drain(); p.pump();
    ...
    {
        ewi::DispatchProbe p; reset(p); p.pump(); p.stop(); drain();
        p.pump(); REQUIRE(p.stats.completed == 0);
        p.start(); p.pump(); REQUIRE(p.stats.stale == 1); drain(); p.pump();
    }
    {
        ewi::DispatchProbe p; reset(p); reject = true; p.pump();
        now = 20000; p.pump(); REQUIRE(calls == 1 && p.stats.rejected == 1 && p.stats.disabled);
    }
    {
        ewi::DispatchProbe p; reset(p); sample.gameThread = false; p.pump(); drain(); p.pump();
    ...
    {
        ewi::DispatchProbe p; reset(p); sample.initialized = false; p.pump(); drain(); p.pump();
        REQUIRE(p.stats.uninitialized == 1 && p.stats.gameThread == 0 && !p.stats.disabled);
    }
    {
        ewi::DispatchProbe p; reset(p);
