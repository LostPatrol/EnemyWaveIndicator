...
ewi::WorldKind worldKind(void* value) {
    return value == &worlds[11] ? ewi::WorldKind::Excluded
        : value == &worlds[10] ? ewi::WorldKind::SpaceRig : ewi::WorldKind::Mission;
}
...
ewi::WorldResult worldOf(void* viewport) {
    if (viewport != &viewportToken) badArguments = true;
    return {world, bridgeFault};
}
void* spawn(void* w, void* cls, const ewi::Position3* p) {
    ++spawns;
...
void* findHubFunction(const ewi::WideView* value) {
    ++hubFinds;
    if (equal(value, ewi::ModHubSearchPath)) return &searchFunction;
    if (equal(value, ewi::ModHubRefreshPath)) return &refreshFunction;
    badArguments = true; return nullptr;
}
...
void setup(ewi::PresentationBootstrap& p) {
...
int main() {
    {
        ...
        REQUIRE(ewi::rescanAndRefreshModHub({findHubFunction, valid, parameterSize, processHub}, &actorToken));
    ...
    {
        ewi::PresentationBootstrap p; setup(p); world = &worlds[10];
        p.tick();
        REQUIRE(p.kind == ewi::WorldKind::SpaceRig && loads == 1 && preparations == 1 && spawns == 1 && refreshCalls == 1 && !badArguments);
        // A preloaded class from an integration entry still receives the native binding on mission travel.
        world = &worlds[0]; p.tick();
        REQUIRE(p.kind == ewi::WorldKind::Mission && loads == 1 && preparations == 2 && spawns == 2 && !badArguments);
    }
    {
        // Mod Hub may not exist on the first post-spawn callback; retry without duplicating Init.
        ewi::PresentationBootstrap p; setup(p); refreshFails = true;
        p.tick();
    ...
    }
    {
        ...
        ewi::PresentationBootstrap p; setup(p); refreshFails = true;
        p.tick();
        for (unsigned i = 1; i < ewi::PresentationBootstrap::MaxRefreshAttempts; ++i) p.tick();
        REQUIRE(p.status == 8 && refreshCalls == ewi::PresentationBootstrap::MaxRefreshAttempts && spawns == 1);
        for (int i = 0; i < 100; ++i) p.tick();
        REQUIRE(refreshCalls == ewi::PresentationBootstrap::MaxRefreshAttempts && spawns == 1);
    }
    {
        REQUIRE(ewi::classifyWorldName(L"", 0) == ewi::WorldKind::Excluded);
        const wchar_t* names[] = {
        ...
        for (int i = 0; i < 5; ++i) {
            const auto expected = i < 3 ? ewi::WorldKind::Excluded : i == 3 ? ewi::WorldKind::SpaceRig : ewi::WorldKind::Mission;
            REQUIRE(ewi::classifyWorldName(names[i], wcslen(names[i])) == expected);
            REQUIRE(ewi::classifyWorldName(names[i], wcslen(names[i]) - 1) == ewi::WorldKind::Excluded);
        }
        ewi::PresentationBootstrap p; setup(p); world = &worlds[11];
        for (int i = 0; i < 100; ++i) p.tick();
    ...
    }
    {
        ...
        ewi::ActiveWorld reader; viewportSearches = 0; viewportMissing = bridgeFault = false;
    ...
    {
        ewi::ActiveWorld reader; viewportMissing = true; viewportSearches = 0;
        reader.configure({findViewport, valid, worldOf});
        for (int i = 0; i < 100; ++i) REQUIRE(reader.read() == nullptr);
        REQUIRE(viewportSearches == 8 && reader.disabled); viewportMissing = false;
    }
    {
        ewi::PresentationBootstrap p; setup(p); worldIsReady = false;
        for (int i = 0; i < 100; ++i) p.tick();
    ...
    for (int failure = 0; failure < 2; ++failure) {
        ewi::PresentationBootstrap p; setup(p); loadFails = failure == 0; prepareFails = failure == 1;
        for (unsigned i = 0; i < ewi::PresentationBootstrap::MaxPreparationAttempts; ++i) p.tick();
        REQUIRE(p.status == 4 && p.preparationFailures == ewi::PresentationBootstrap::MaxPreparationAttempts);
        for (int i = 0; i < 100; ++i) p.tick();
        REQUIRE(loads == static_cast<unsigned>(failure == 0 ? ewi::PresentationBootstrap::MaxPreparationAttempts : 1)
            && spawns == 0 && !badArguments);
    }
    {
        ewi::PresentationBootstrap p; setup(p); spawnFails = true; p.tick();
        REQUIRE(p.status == 4 && spawns == 1 && !badArguments);
    }
    {
        // A null travel World does not poison the next ready gameplay World.
        ewi::PresentationBootstrap p; setup(p); world = nullptr; p.tick();
    ...
    }
    {
        ...
        ewi::PresentationBootstrap p; setup(p); p.tick();
    ...
    }
    {
        ...
        ewi::PresentationBootstrap p; setup(p); p.tick();
        for (unsigned i = 1; i < ewi::PresentationBootstrap::MaxRecoveryAttempts; ++i) {
            controllerExists = false; p.tick();
        }
        controllerExists = false; p.tick();
        REQUIRE(p.status == 4 && spawns == ewi::PresentationBootstrap::MaxRecoveryAttempts);
        for (int i = 0; i < 100; ++i) p.tick();
        REQUIRE(spawns == ewi::PresentationBootstrap::MaxRecoveryAttempts && !badArguments);
    }
