...
$catalogRelative='engine\Authoring\EwiAuthoring\Source\EwiAuthoring\Public\EwiWaveTypes.h'
...
$names=@('BP_EwiAuto','BP_EwiResources','BP_EwiPulse','WBP_EwiMarker','SG_EwiSettings','WBP_EwiSettings','M_EwiRedPulse','InitCave','InitSpacerig')
...
$audit=Join-Path $cookDir 'dependency-audit.json';@{passed=$true;contentOnly=$false;nativeAbi=65536;assets=9} | ConvertTo-Json | Set-Content $audit
$native=@{ProbeVersion='1.0.0';OfflinePassed=$true;CaptureOfflinePassed=$true;DispatchOfflinePassed=$true;PresentationOfflinePassed=$true;SourceFiles=@(Get-FileHash $source);SHA256=(Get-FileHash $dll).Hash}
...
$cook=@{Success=$true;ContentOnly=$false;RuntimeDllRequired=$true;Version='1.0.0';AssetsOnly=$true;ContainsGameAssetCopies=$false;InlineMaterialShaders=$true;PakHashesVerified=$true;Files=$files;Verification=$assetsDir;Pak=Get-FileHash $pak;DependencyAudit=Get-FileHash $audit;PackagingConfig=Get-FileHash $config}
...
$native.ProbeVersion='0.6.0';Save-Fixture;Assert-Rejected 'Missing native validation';$native.ProbeVersion='1.0.0'
...
if($m.ArchiveName -ne 'EnemyWaveIndicator-1.0.0.zip'){throw 'Unexpected public archive name.'}
...
$publicArchive=Join-Path $testRoot 'dist\EnemyWaveIndicator-1.0.0.zip'
...
if($archives.Count -ne 1 -or $archives[0].Name -ne 'EnemyWaveIndicator-1.0.0.zip'){throw 'Unexpected public archive set.'}
