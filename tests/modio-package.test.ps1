...
$audit=Join-Path $cookDir 'dependency-audit.json';@{passed=$true;contentOnly=$false;nativeAbi=590592;assets=9} | ConvertTo-Json | Set-Content $audit
$native=@{ProbeVersion='0.9.3';OfflinePassed=$true;CaptureOfflinePassed=$true;DispatchOfflinePassed=$true;PresentationOfflinePassed=$true;SourceFiles=@(Get-FileHash $source);SHA256=(Get-FileHash $dll).Hash}
...
$cook=@{Success=$true;ContentOnly=$false;RuntimeDllRequired=$true;Version='0.9.3';AssetsOnly=$true;ContainsGameAssetCopies=$false;InlineMaterialShaders=$true;PakHashesVerified=$true;Files=$files;Verification=$assetsDir;Pak=Get-FileHash $pak;DependencyAudit=Get-FileHash $audit;PackagingConfig=Get-FileHash $config}
...
$native.ProbeVersion='0.6.0';Save-Fixture;Assert-Rejected 'Missing native validation';$native.ProbeVersion='0.9.3'
...
if($m.ArchiveName -ne 'EnemyWaveIndicator-0.9.3.zip'){throw 'Unexpected public archive name.'}
...
$publicArchive=Join-Path $testRoot 'dist\EnemyWaveIndicator-0.9.3.zip'
...
if($archives.Count -ne 1 -or $archives[0].Name -ne 'EnemyWaveIndicator-0.9.3.zip'){throw 'Unexpected public archive set.'}
