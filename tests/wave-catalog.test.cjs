...
const header=fs.readFileSync(path.join(root,'engine/Authoring/EwiAuthoring/Source/EwiAuthoring/Public/EwiWaveTypes.h'),'utf8');
const settings=fs.readFileSync(path.join(root,'engine/Authoring/EwiAuthoring/Source/EwiAuthoring/Private/EwiSettings.inl'),'utf8');
...
test('all concrete audited controller classes have one stable native/UI source entry',()=>{
 assert.equal(catalog[0].key,'Normal');assert.equal(catalog.length,47);
...
test('Simplified Chinese UI wave names exactly follow WAVE-TYPES while marker defaults stay English',()=>{
 ...
 assert.match(settings,/FString\(ewi::WaveTypes/);
