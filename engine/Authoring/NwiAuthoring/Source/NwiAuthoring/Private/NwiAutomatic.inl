...
void BuildAutomatic()
{
    auto* Parent = LoadClass<AActor>(nullptr, TEXT("/Game/NormalWaveIndicator/BP_NwiVisualTest.BP_NwiVisualTest_C"));
    ...
    Variable(BP, TEXT("NativeAbi"), Type(UEdGraphSchema_K2::PC_Int), TEXT("393216"));
    Variable(BP, TEXT("NativeCookie"), Type(UEdGraphSchema_K2::PC_Int), TEXT("0"));
    ...
    for (int32 I = 0; I < 8; ++I)
    {
        Variable(BP, *FString::Printf(TEXT("NativePoint%d"), I), Type(UEdGraphSchema_K2::PC_Struct, TBaseStructure<FVector>::Get()));
        for (auto* Prefix : { TEXT("NativeSerial"), TEXT("NativeVisible"), TEXT("AppliedSerial"), TEXT("AppliedVisible") })
    ...
    auto* G = Graph(BP);
    auto* Poll = Custom(G, TEXT("NwiPoll")); // Zero parameters; the native module binds only this own no-op function.
    (void)Poll;
    auto* Service = Custom(G, TEXT("ServiceNative"));
    ...
    for (int32 I = 0; I < 8; ++I) {
        Apply[I] = Custom(G, *FString::Printf(TEXT("ApplyNative%d"), I));
        Hide[I] = Custom(G, *FString::Printf(TEXT("HideNative%d"), I));
    ...
    auto* TickGate = Branch(G, CanRun, TEXT("ReturnValue")); Link(Tick, TEXT("then"), TickGate, TEXT("execute"));
    ...
    auto* CallService = Call(G, BP->GeneratedClass, TEXT("ServiceNative")); Link(Refresh, TEXT("then"), CallService, TEXT("execute"));
    ...
    UEdGraphNode* ServiceExec = Call(G, BP->GeneratedClass, TEXT("NwiPoll")); Link(Service, TEXT("then"), ServiceExec, TEXT("execute"));
    ...
    for (int32 I = 0; I < 8; ++I)
    {
        ...
        const FString PointName = FString::Printf(TEXT("NativePoint%d"), I), SerialName = FString::Printf(TEXT("NativeSerial%d"), I);
        const FString VisibleName = FString::Printf(TEXT("NativeVisible%d"), I), AppliedName = FString::Printf(TEXT("AppliedSerial%d"), I);
        const FString ShownName = FString::Printf(TEXT("AppliedVisible%d"), I), HideName = FString::Printf(TEXT("HideNative%d"), I);
        ...
        GetDefault<UEdGraphSchema_K2>()->TrySetDefaultText(*Pin(Label, TEXT("Label")), FText::FromString(TEXT("[!] NORMAL WAVE")));
        ...
        auto* ApplyCall = Call(G, BP->GeneratedClass, *FString::Printf(TEXT("ApplyNative%d"), I)); Link(ServiceExec, TEXT("then"), ApplyCall, TEXT("execute")); ServiceExec = ApplyCall;
        ...
        auto* NativeVisible = Get(G, *VisibleName); auto* IsVisible = Call(G, UKismetMathLibrary::StaticClass(), TEXT("Greater_IntInt")); Link(NativeVisible, *VisibleName, IsVisible, TEXT("A"));
        auto* ShowGate = Branch(G, IsVisible, TEXT("ReturnValue")); Link(SaveSerial, TEXT("then"), ShowGate, TEXT("execute"));
        ...
        Link(Hud, *HudName, Location, TEXT("self")); Link(Point, *PointName, Location, TEXT("WorldLocation")); Link(Move, TEXT("then"), Location, TEXT("execute"));
        ...
        auto* Timer = Call(G, UKismetSystemLibrary::StaticClass(), TEXT("K2_SetTimer")); Link(Self, TEXT("self"), Timer, TEXT("Object")); Value(Timer, TEXT("FunctionName"), *HideName); Link(Get(G, TEXT("DurationSec")), TEXT("DurationSec"), Timer, TEXT("Time"));
