...
# Presentation authoring — 1.0.0

Open FSD.uproject in UE4.27.2. FSD is the required content mount name, not a shipped native module. The editor-only EwiAuthoring plugin emits nine production assets: Auto, Resources, Pulse, Marker, Settings page, Settings SaveGame, Red Material, InitCave and InitSpacerig.

Auto is an always-relevant replicated actor created by host initializers. The native DLL binds only Auto.EwiPoll (zero arguments), validates NativeAbi=0x10000, and writes owned region fields. Clients skip native polling and render replicated regions. Clock expiry uses server world time; display preferences and visual pools remain local. Each pulse owns one reusable MID and has no collision, gameplay navigation or replication.
...
Prepare-ModHubDevkit.ps1 installs pinned development interface references. Never package _ModHub, EwiValidation, editor binaries or original game assets. The cook admits exactly 18 owned files and verifies their Pak hashes. The obsolete FSD reflection stub and approximate content-capture graph have been removed. BP_EwiVisualTest remains an unshipped resource/display regression fixture, not Auto's parent.
