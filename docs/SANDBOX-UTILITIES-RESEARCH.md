...
**范围更新：** 本文保留原“精准识别自然潮”的研究结论。用户随后明确接受所有已通知敌人生成点位及近似事件分类；0.7.0 已按新范围改用现有生成事件和纯蓝图初始化，不再需要游戏开发方提供新接口。当前实现与验收状态见 [README](../README.md) 和 [发布流程](RELEASE.md)，不要将本文旧范围的未实现结论套用到新范围，也不要宣称新版本攻克了精准来源识别。
