...
if ($native.ProbeVersion -ne '0.9.0' -or !$native.OfflinePassed -or !$native.CaptureOfflinePassed -or !$native.DispatchOfflinePassed -or !$native.PresentationOfflinePassed) { throw 'Missing native validation.' }
if (!$cook.Success -or $cook.ContentOnly -or !$cook.RuntimeDllRequired -or $cook.Version -ne '0.9.0' -or !$cook.AssetsOnly -or $cook.ContainsGameAssetCopies -or !$cook.InlineMaterialShaders -or !$cook.PakHashesVerified -or $cook.Files.Count -ne 18) { throw 'Missing native-Pak cook validation.' }
...
$output=Join-Path $root ('agent\codex\mintcat-0.9.0-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
...
@{
    Version='0.9.0'; DistributionTarget='MintCat'; Status='36 stock wave types test candidate; real-game and network validation pending'
