...
if ($native.ProbeVersion -ne '0.8.0' -or !$native.OfflinePassed -or !$native.CaptureOfflinePassed -or !$native.DispatchOfflinePassed -or !$native.PresentationOfflinePassed) { throw 'Missing native validation.' }
if (!$cook.Success -or $cook.ContentOnly -or !$cook.RuntimeDllRequired -or $cook.Version -ne '0.8.0' -or !$cook.AssetsOnly -or $cook.ContainsGameAssetCopies -or !$cook.InlineMaterialShaders -or !$cook.PakHashesVerified -or $cook.Files.Count -ne 18) { throw 'Missing native-Pak cook validation.' }
...
if (!$audit.passed -or $audit.contentOnly -or $audit.nativeAbi -ne 524288 -or $audit.assets -ne 9) { throw 'Invalid dependency audit.' }
$output=Join-Path $root ('agent\codex\mintcat-0.8.0-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
...
@{
    Version='0.8.0'; DistributionTarget='MintCat'; Status='natural-wave test candidate; task1 incomplete'
    RuntimeDllRequired=$true; ModioSubscriptionOnly=$false; MintCatInstallTested=$false; NetworkTested=$false; ReleaseReady=$false
    Capture='six exact scheduler/source return addresses; accepted queue item; successful actor; regular registration required'
    ...
    SupportedSources=@('Natural'); UnsupportedRequestedFeatures=@('all other source types','per-type configuration beyond natural','multi-second exact prediction')
    ClientRequiresPak=$true; HostRequiresDll=$true; MaximumRegions=8
    Requires=@('MintCat with UE4SSL.JavaScript stable 0.31.0 audited runtime','Mod Hub','matching 0.8.0 Pak on participating clients')
