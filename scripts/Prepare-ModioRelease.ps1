...
if (!$cook.Success -or !$cook.ContentOnly -or $cook.Version -ne '0.7.1' -or $cook.SeparateNativeBootstrapRequired -ne $false -or
...
if (!$assets.Success -or !$assets.Validation.success -or !$assets.Validation.capture_test -or !$assets.Validation.small_enemy_filter_test -or !$assets.Validation.content_only -or
...
$output = Join-Path $root ('agent\codex\modio-0.7.1-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
...
@{
    Version='0.7.1'; DistributionTarget='Modio'; Status='content-only beta candidate'
    ...
    ExcludedBuckets=@('ActiveSwarmerEnemies','ActiveCritters'); RuntimeDllRequired=$false; HostOnly=$true; MaximumRegions=8; MergeRadiusMeters=8
