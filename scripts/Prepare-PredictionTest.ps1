...
if (!$native.PredictionTest -or $native.ProbeVersion -ne '0.9.4-prediction-test.3' -or !$native.OfflinePassed -or !$native.PredictionOfflinePassed) {
...
$output = Join-Path $root ('agent\codex\EnemyWaveIndicator-prediction-test-3-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
...
@{
    Version = '0.9.4-prediction-test.3'; Experimental = $true; Published = $false; Installed = $false
