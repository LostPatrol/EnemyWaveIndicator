...
if (!$native.PredictionTest -or $native.ProbeVersion -ne '0.9.4-prediction-test.5' -or !$native.OfflinePassed -or !$native.PredictionOfflinePassed) {
...
$output = Join-Path $root ('agent\codex\EnemyWaveIndicator-prediction-test-5-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
...
@{
    Version = '0.9.4-prediction-test.5'; Experimental = $true; Published = $false; Installed = $false
    Behavior = 'One T-5 sample; stock all-nav-points query plus up to eight deterministic shell candidates; no spawn or random-point call.'
