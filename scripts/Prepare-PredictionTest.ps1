...
if (!$native.PredictionTest -or $native.ProbeVersion -ne '0.9.4-prediction-test.4' -or !$native.OfflinePassed -or !$native.PredictionOfflinePassed) {
...
$output = Join-Path $root ('agent\codex\EnemyWaveIndicator-prediction-test-4-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
...
@{
    Version = '0.9.4-prediction-test.4'; Experimental = $true; Published = $false; Installed = $false
