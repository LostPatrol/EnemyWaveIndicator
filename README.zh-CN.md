...
Enemy Wave Indicator 会用发光球体、可自定义文字和距离信息，标出检测到的敌人生成区域。目前支持《Deep Rock Galactic》的自然潮，以及 38 种游戏自带的脚本虫潮。

> 本项目目前是 Windows 公开测试版，适配 Steam 版 DRG 1.40（已测试游戏版本 build 24903151）。

## 功能

- 支持自然潮和 38 种脚本虫潮，每种类型都有独立的开关和提示文字；新增三提石矿藏、矿化爆发和氪石感染的直接事件刷怪。
...
0.9.4 新增 8 种任务警告／异变来源的精确生成中心，包括蜂拥浩劫、凝血化糖、强敌环伺、矿化爆发（任务警告）和诡异洞穴。默认开启全部虫潮播报，只有**深掘钻梯**、**执勤护送：钻进**、**核心岩事件**、**核心侵扰警告**和**诡异洞穴**默认关闭；警示文字默认以黄—红两色闪烁，球体 RGB 通道使用 0–1 的标准化范围。在 Mod Hub 中修改任意选项后，请点击**应用并保存**。全部类型见[虫潮类型清单](docs/WAVE-TYPES.md)。

![自然潮标记](docs/media/normal_wave_1.png)

## 安装方法

0.9.4 对应的 MintCat 压缩包目前仅为本地测试候选，包含 `main.dll`、`EnemyWaveIndicator_P.pak` 和再分发依赖的许可证。正式发布的最新版本仍可从 [GitHub Releases](https://github.com/LostPatrol/EnemyWaveIndicator/releases) 下载；原生 DLL 与 Pak 不得混用版本。
...
- **Mod Hub 中没有出现本 Mod：**确认 Mod Hub 已启用，并确认空间站已经完成加载。当前 0.9.4 测试包没有固定启动延时，且 Mod Hub 注册接口使用与 0.9.0 已验证版本一致的稳定静态元数据。若压缩包的 SHA-256 与测试清单不同，请关闭游戏后在 MintCat 中重新导入。
