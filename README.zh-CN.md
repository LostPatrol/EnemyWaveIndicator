...
- **Mod Hub 中没有出现本 Mod：**确认 Mod Hub 已启用，并确认空间站已经完成加载。当前 0.9.1 包没有固定启动延时：首次条件检查通过时便会立即初始化，随后刷新 Mod Hub 的模组 Actor 列表并重建可见页面。若使用的是本次热修复前构建的压缩包，请在 MintCat 中重新导入。
