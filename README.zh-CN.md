...
Enemy Wave Indicator 会用发光球体、可自定义文字和距离信息，标出检测到的敌人生成区域。目前支持《Deep Rock Galactic》的自然潮，以及 46 种游戏自带的脚本虫潮。
...
0.9.5 修复监听服务器房主在其他玩家加入后可能完全没有标记控件的问题。HUD 所有者现在不再依赖控制器列表顺序，并且未完整构建的标记池可以安全重试。默认开启全部虫潮播报，只有**深掘钻梯**、**执勤护送：钻进**、**核心岩事件**、**核心侵扰警告**和**幽魂不散**默认关闭。在 Mod Hub 中修改任意选项后，请点击**应用并保存**。全部类型见[虫潮类型清单](docs/WAVE-TYPES.md)。

![自然潮标记](docs/media/normal_wave_1.png)

## 安装方法

0.9.5 对应的 MintCat 压缩包包含 `main.dll`、`EnemyWaveIndicator_P.pak` 和再分发依赖的许可证，可从 [GitHub Releases](https://github.com/LostPatrol/EnemyWaveIndicator/releases) 下载；原生 DLL 与 Pak 不得混用版本。
...
- **Mod Hub 中没有出现本 Mod：**确认 Mod Hub 已启用，并确认空间站已经完成加载。0.9.5 没有固定启动延时，并使用稳定的静态 Mod Hub 注册元数据。若压缩包的 SHA-256 与发布校验值不同，请关闭游戏后在 MintCat 中重新导入。
