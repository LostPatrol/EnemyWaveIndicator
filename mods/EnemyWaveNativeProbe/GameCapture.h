// Native capture contract. Production hooks only observe; the opt-in prediction build may replace one audited normal-wave center.
#pragma once
#include "SpawnAttribution.h"
#include <array>
#include "../../engine/Authoring/EwiAuthoring/Source/EwiAuthoring/Public/EwiWaveTypes.h"
...
struct Binding {
    ...
    std::array<uintptr_t, 6> sourceChain{}; // Includes the normal scheduler, not only its shared spawning helper.
    uint32_t threadId = 0;
    Target pool, location, group, spread, spreadCallback, center;
    Target normalCenter; uintptr_t normalCenterReturn = 0;
    ...
    // Test-only gate: return true to replace the exact normal selector call with an already sampled center.
    bool (*consumeNormalOverride)(const SpawnKey& origin, float radius, SpawnKey& output) noexcept = nullptr;
};
struct Stats {
    ...
    uint64_t normalCenterSamples = 0, normalCenterCalls = 0, normalCenterOverrides = 0, normalCenterFallbacks = 0;
